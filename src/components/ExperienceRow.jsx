  ...
  return (
    <div className="max-w-[900px] mx-auto flex flex-col sm:flex-row divide-y sm:divide-y-0 sm:divide-x divide-black/10 dark:divide-white/10">
      {currentItems.map((item, index) => (
        <div key={index} className="flex-1 flex flex-col items-center text-center gap-2 py-6 sm:py-0 sm:px-8">
          ...
          <p data-locale-fade className="text-xs text-slate-500 dark:text-slate-400 font-medium">{item.date}</p>
          <p data-locale-fade className="text-sm text-neutral-700 dark:text-neutral-300 font-medium">{item.role}</p>
        </div>
      ))}
    </div>
  );
